"""
Description: Optimized PySpark script for processing iOS accelerometer data sequentially by participant
"""
...
OUTPUT_TABLE = "derived_accelerometer_ios_pyspark_" + datetime.now().strftime("%Y%m%d")
COMPOSITE_KEYS = ["_id"]
PROCESSING_CONFIG = {
    "pa_threshold": 0.4,
    "batch_size": 50  # Number of participants to process at once
}
...
def process_participant_batch(spark, config, logger, participant_ids, timezone_df):
    ...
    # Filter for plausible g values (iOS data is already in g)
...
def main():
    ...
    logger = setup_logger("IOSAccelerometerProcessor")
    logger.info("Starting optimized iOS accelerometer processing.")
    try:
        config = load_config()
        spark = create_spark_session("IOSAccelerometerProcessor_Optimized")
        ...
        for i in range(0, total_participants, batch_size):
            ...
            logger.info(f"Processing batch {i//batch_size + 1}/{(total_participants//batch_size)+1}")
...
spark-submit \
  ...
  /home/prositadmin/dev_workspace/src/W_create_derived_accelerometer_ios_pyspark.py \
  > /home/prositadmin/dev_workspace/logs/W_create_derived_accelerometer_ios_pyspark.log 2>&1 &


tail -f /home/prositadmin/dev_workspace/logs/W_create_derived_accelerometer_ios_pyspark.log
cat /home/prositadmin/dev_workspace/logs/W_create_derived_accelerometer_ios_pyspark.log
rm /home/prositadmin/dev_workspace/logs/W_create_derived_accelerometer_ios_pyspark.log
'''
