/*
============================================================================
Function : create_feature_daily_accelerometer_android_timeofday_4parts_psql

Purpose  : For a given study schema, computes daily Android accelerometer
...
DROP FUNCTION IF EXISTS create_feature_daily_accelerometer_android_timeofday_4parts_psql(TEXT);

CREATE OR REPLACE FUNCTION create_feature_daily_accelerometer_android_timeofday_4parts_psql(
...
BEGIN
    ...
    EXECUTE format($sql$
        CREATE TABLE IF NOT EXISTS %I.feature_daily_activity_android_timeofday_4parts_psql_20250818 (
    ...
    -- Populate table with daily aggregates
    EXECUTE format($sql$
        WITH base_data AS (
            SELECT 
                participantid,
                COALESCE(
                    CASE
                        WHEN measuredat_local LIKE '%%T%%' THEN TO_TIMESTAMP(REPLACE(measuredat_local, 'T', ' '), 'YYYY-MM-DD HH24:MI:SS.MS')
                        WHEN measuredat_local LIKE '____-__-__ __:__:__' THEN TO_TIMESTAMP(measuredat_local, 'YYYY-MM-DD HH24:MI:SS')
                        ELSE NULL
                    END,
                    CASE
                        WHEN measuredat_utc LIKE '%%T%%' THEN TO_TIMESTAMP(REPLACE(measuredat_utc, 'T', ' '), 'YYYY-MM-DD HH24:MI:SS.MS')
                        WHEN measuredat_utc LIKE '____-__-__ __:__:__' THEN TO_TIMESTAMP(measuredat_utc, 'YYYY-MM-DD HH24:MI:SS')
                        ELSE NULL
                    END
                ) AS ts,
                non_vigorous_pa::DOUBLE PRECISION AS non_vigorous_pa,
                vigorous_pa::DOUBLE PRECISION AS vigorous_pa,
                euclidean_norm_g::DOUBLE PRECISION AS euclidean_norm
            FROM %I.derived_accelerometer_android_pyspark_20250808
            WHERE (euclidean_norm_g::DOUBLE PRECISION) BETWEEN 0 AND 20
        ),
        ...
        INSERT INTO %I.feature_daily_activity_android_timeofday_4parts_psql_20250818 (
            participantid,
            date_local,
            weekday_local,
    ...
    $sql$, study_schema_name, study_schema_name);
END;
$$ LANGUAGE plpgsql;

-- Example usage:
-- SELECT create_feature_daily_accelerometer_android_timeofday_4parts_psql('user1_workspace');