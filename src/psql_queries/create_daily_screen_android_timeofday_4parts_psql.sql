/*
====================================================================================
Function : create_feature_daily_screen_android_timeofday_4parts_psql_20250818
 
Purpose  :
    Computes Android daily screen time split into 4 time-of-day buckets using a 6AM–6AM cycle.
====================================================================================
*/

-- Drop the function if it exists
DROP FUNCTION IF EXISTS create_feature_daily_screen_android_timeofday_4parts_psql_20250818(TEXT);

CREATE OR REPLACE FUNCTION create_feature_daily_screen_android_timeofday_4parts_psql_20250818(
...
BEGIN
    ...
    EXECUTE format($sql$
        CREATE TABLE IF NOT EXISTS %I.feature_daily_screen_android_timeofday_4parts_psql_20250818 (
    ...
    WITH base_events AS (
        SELECT 
            ...
            CASE 
                ...
                THEN CAST(event_start_time_local AS timestamp)::date
                ELSE (CAST(event_start_time_local AS timestamp)::date - INTERVAL '1 day')::date
            END AS date_local,
            TRIM(TO_CHAR(
                CASE 
                    WHEN CAST(event_start_time_local AS timestamp)::time >= TIME '06:00:00'
        ...
        FROM %I.derived_events_screen_android_pyspark_20250818
        WHERE screen_on_time_in_secs::integer <= 10800
    ),
    ...
    INSERT INTO %I.feature_daily_screen_android_timeofday_4parts_psql_20250818 (
        participantid,
        date_local,
        weekday_local,
...
-- To run:
SELECT create_feature_daily_screen_android_timeofday_4parts_psql_20250818('user1_workspace');


-- ============================================================================
-- Simple Test Query for 4-Bucket Screen Time Function
...
FROM user1_workspace.feature_daily_screen_android_timeofday_4parts_psql_20250818;
